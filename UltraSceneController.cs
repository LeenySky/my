            ...
            try
            {
                ...
                for (int i = 0; i < allAtoms.Count; i++)
                {
                    ...
                    if (pCount <= 4)
                    {
                        ...
                        // JEDYNY POPRAWNY SPOSÓB BLOKOWANIA AUTO BEHAVIORS PRZEZ GETPARAM
                        JSONStorable geoComponent = atom.GetStorableByID("geometry");
                        if (geoComponent != null)
                        {
                            // Pobieramy parametry za pomocą uniwersalnej metody GetParam i rzutujemy na JSONStorableBool
                            JSONStorableBool blinkBool = geoComponent.GetParam("blinkEnabled") as JSONStorableBool;
                            if (blinkBool != null && blinkBool.val) blinkBool.val = false;

                            JSONStorableBool exprBool = geoComponent.GetParam("expressionsEnabled") as JSONStorableBool;
                            if (exprBool != null && exprBool.val) exprBool.val = false;
                        }
...
{ 
    private UltraSceneController plugin;
    private Dictionary<string, DAZCharacterSelector> geometryCache = new Dictionary<string, DAZCharacterSelector>();
    
    // Timery i stany do obsługi silnika mrugania
    private Dictionary<string, float> nextBlinkTime = new Dictionary<string, float>();
    private Dictionary<string, float> blinkDuration = new Dictionary<string, float>();
    private Dictionary<string, bool> isBlinking = new Dictionary<string, bool>();

    // Definicje technicznych pakietów morphów z VaM Hub
    ...
    { 
        ...
        // Pobranie lub zbuforowanie komponentu geometrii DAZ
        DAZCharacterSelector geo = null;
        if (!geometryCache.TryGetValue(personAtom.uid, out geo) || geo == null)
        {
            geo = personAtom.GetStorableByID("geometry") as DAZCharacterSelector;
            if (geo != null) geometryCache[personAtom.uid] = geo;
        }
        if (geo == null) return;

        // Skalowanie podniecenia na wartość 0.0 - 1.0
        float arousalFactor = Mathf.Clamp01(currentArousal / 100f);

        // =========================================================================
        // KOMPLETNA DEKLARACJA WSZYSTKICH ZMIENNYCH DLA OBU CZĘŚCI MODUŁU
        // =========================================================================
        float targetDesire = 0f;
        float targetExcitement = arousalFactor; 
        ...
        // Osobne sterowanie powiekami (AshAuryn)
        float targetWinkL = 0f;
        float targetWinkR = 0f;

        // Domyślne parametry mrugania oczu
        float blinkFreqMin = 3.0f;
        float blinkFreqMax = 6.0f;

        // Algorytm falowania języka (14mhz Engine z repozytorium GitHub)
        float tongueFlutter = 0f;
        if (arousalFactor > 0.4f)
        {
            tongueFlutter = Mathf.Sin(Time.time * 8.0f) * 0.4f * arousalFactor;
        }

        // MASZYNA STANÓW: TWOJA REŻYSERIA 12 OSOBOWOŚCI (Część 1)
        switch (personality)
        {
            case "Sensual / Romantic":
                blinkFreqMin = 6.0f; blinkFreqMax = 12.0f; 
                targetDesire = 0.5f + (arousalFactor * 0.5f);
                targetBedroomSmile = 0.4f;
                
                if (arousalFactor >= 0.5f) targetSnarlL = (arousalFactor - 0.5f) * 0.8f; 
                if (currentAction == "Licking" && arousalFactor > 0.5f) targetBedroomSmile += tongueFlutter;
                if (currentAction == "Anal") { targetConfused = 0.4f; targetUhOh = 0.5f; }
                break;

            case "Shy / Surprised":
                blinkFreqMin = 1.5f; blinkFreqMax = 3.5f; 
                targetConfused = 0.4f + (arousalFactor * 0.4f);
                targetSurprise = 0.3f;
                if (currentAction == "Anal") { targetWrongHoleA = 0.7f; targetPain = 0.4f; }
                break;

            case "Passionate / Ecstatic":
                targetDesire = 0.8f;
                if (arousalFactor >= 0.5f) targetSnarlR = 0.5f + ((arousalFactor - 0.5f) * 0.5f);
                break;
            ...
            case "Fear / Shocked":
                blinkFreqMin = 1.0f; blinkFreqMax = 2.0f; 
                targetAfraid = 0.8f;
                targetConfused = 0.6f;
                targetUhOh = 0.7f;
                break;
            ...
            case "Horny / Disgust":
                targetDesire = 0.7f;
                if (arousalFactor >= 0.5f && currentAction != "Anal") targetSnarlL = 0.5f;
                if (currentAction == "Anal") { targetContempt = 0.8f; targetUgh = 0.6f; }
                break;
            case "Horny / Surprised":
                targetHappy = 0.5f;
                targetSurprise = 0.4f;
                if (arousalFactor < 0.5f && UnityEngine.Random.value < 0.02f) targetWinkR = 1.0f;
                if (arousalFactor >= 0.5f) targetSnarlR = 0.4f; 
                break;

            case "Horny / Pain":
                targetDesire = 0.6f;
                targetPain = 0.3f + (arousalFactor * 0.4f);
                targetLipBiteWide = 0.7f; 
                if (arousalFactor >= 0.5f) 
                {
                    targetSnarlL = 0.6f + ((arousalFactor - 0.5f) * 0.4f);
                    targetWrongHoleB = 0.3f;
                }
                break;

            case "Evil / Horny":
                blinkFreqMin = 8.0f; blinkFreqMax = 15.0f; 
                targetDesire = 0.8f;
                if (arousalFactor >= 0.5f) targetSnarlL = 0.7f; 
                break;
            ...
            case "Great / Surprised":
                targetHappy = 0.8f; 
                targetSurprise = 0.2f;
                if (arousalFactor < 0.6f && UnityEngine.Random.value < 0.03f) 
                {
                    if (UnityEngine.Random.value > 0.5f) targetWinkL = 1.0f; else targetWinkR = 1.0f;
                }
        ...
        // =========================================================================
        // DYNAMICZNE MAPOWANIE ZMIENNYCH DLA SPECJALNYCH PROFILI PSYCHOLOGICZNYCH
        // =========================================================================
        targetAfraid = (personality == "Fear / Shocked") ? 0.8f : 0f;
        targetAngry = (personality == "Angry / Surprised" && arousalFactor < 0.5f) ? 0.7f : 0f;
        targetConcentrate = (personality == "Concerned / Surprised") ? 0.5f : 0f;
        ...
        {
            // ==========================================
            // LOGIKA DYNAMICZNEGO MRUGANIA (Blinking Engine)
            // ==========================================
            if (!nextBlinkTime.ContainsKey(personAtom.uid)) nextBlinkTime[personAtom.uid] = Time.time + UnityEngine.Random.Range(blinkFreqMin, blinkFreqMax);
            if (!isBlinking.ContainsKey(personAtom.uid)) isBlinking[personAtom.uid] = false;
            if (!blinkDuration.ContainsKey(personAtom.uid)) blinkDuration[personAtom.uid] = 0f;

            if (Time.time >= nextBlinkTime[personAtom.uid] && !isBlinking[personAtom.uid])
            {
                isBlinking[personAtom.uid] = true;
                blinkDuration[personAtom.uid] = Time.time + 0.12f; 
            }

            if (isBlinking[personAtom.uid])
            {
                if (Time.time < blinkDuration[personAtom.uid])
                {
                    float blinkWeight = (personality == "Passionate / Ecstatic" || currentArousal > 80f) ? 0.6f : 1.0f;
                    if (targetWinkL < 0.1f) targetWinkL = blinkWeight;
                    if (targetWinkR < 0.1f) targetWinkR = blinkWeight;
                }
                else
                {
                    isBlinking[personAtom.uid] = false;
                    nextBlinkTime[personAtom.uid] = Time.time + UnityEngine.Random.Range(blinkFreqMin, blinkFreqMax);
                }
            }
        }

        // 5. Pobranie głównego storable geometrii i aplikowanie suwaków przez GetParam
        ...
        if (geoComponent != null)
        {
            ...
            SetMorphValueSafe(geoComponent, "Fear", targetAfraid * 0.8f); 
            ...
            SetMorphValueSafe(geoComponent, "Shock", targetAfraid * 0.9f); 
            ...
            // Aplikowanie morphów z paczki公ascorad (.vmi)
            ...
            // Aplikowanie precyzyjnego mrugania oczu z paczki AshAuryn (.vmi)
    ...
    // Oficjalna metoda sterowania suwakami oparta na GetParam i rzutowaniu na JSONStorableFloat
    private void SetMorphValueSafe(JSONStorable geo, string morphUid, float value)
    {
        try
        {
            if (geo != null)
            {
                // Wyciągamy morph dokładnie jako parametr float z poziomu storable
                JSONStorableFloat morphParam = geo.GetParam(morphUid) as JSONStorableFloat;
                if (morphParam != null)
                {
        ...
        catch { /* Bezpieczny ratunek przed brakiem suwaka */ }
    }
}
